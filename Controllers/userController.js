...
//Has Protect & Strict MWs before it
...
const me = asyncErHandler(async(req,res)=>{
    // console.log()
    if(!req.user){
        console.log("⚠ User not logged in! Fix protect middleware now! ⚠")
...
const getSaved = asyncErHandler(async(req,res)=>{
    if(!req.user){
        console.log("⚠ User not logged in! Fix protect middleware now! ⚠")
        return res.status(401).json({
            status:'fail',
...
module.exports = {updatePassword,updateMe,deleteMe,getAllUsers,me,getSaved};